...
/**
 * Handle SDK result messages: session ID capture, error classification,
 * and result deferral when teammates are still active.
 */
export function handleResultMessage(resultMsg: any, state: SessionStreamState, enqueue: (data: Uint8Array) => boolean): void {
  ...
  if ((resultMsg.subtype === 'error' || resultMsg.is_error) && !hasCompletedTeammates) {
    ...
    const errorEvent: StreamEventData = {
      ...
      sessionId: sid, errorCode: classified.code,
    };
