...
export function handleSystemMessage(sysMsg: any, state: SessionStreamState, enqueue: (data: Uint8Array) => boolean): void {
  ...
  if (
    ...
    if (existingIdx !== undefined) {
      ...
      state.log.info('Teammate bridged (toolUseId → taskId)', { toolUseId: sysMsg.tool_use_id, taskId: sysMsg.task_id });
    } else {
      ...
      state.log.info('Teammate started (sole source)', { taskId: sysMsg.task_id, taskType: sysMsg.task_type });
    }
  }
  ...
  if (sysMsg.subtype === 'task_notification' && sysMsg.task_id) {
    ...
    if (!wasActive) {
      state.log.info('Skipping duplicate task_notification (already completed via tool_result)', { taskId: sysMsg.task_id });
    } else {
      const existingIdx = state.teammateIndexMap.get(sysMsg.task_id);
      if (existingIdx !== undefined) {
        const originalStart = state.teammateStartTimes.get(sysMsg.task_id) ?? Date.now();
