...
export function SessionSidebar({
  ...
  useEffect(() => {
    if (mode !== 'agent') { prevStreamingIds.current = new Set(); return; }
  ...
  const handleRenameGroup = useCallback(async (gid: string, name: string) => {
    await fetch(`/api/groups/${gid}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) });
  ...
  const patchGroup = useCallback(async (url: string, body: object) => {
  ...
  const toggleGroup = useCallback((gid: string) => {
    setCollapsedGroups((p) => { const n = new Set(p); n.has(gid) ? n.delete(gid) : n.add(gid); return n; });
  }, []);

  const filtered = searchQuery ? sessions.filter((s) => s.title.toLowerCase().includes(searchQuery.toLowerCase())) : sessions;
  ...
  const rowProps = (s: SessionMetadata, extra?: { indent?: boolean }) => ({
    ...
    onDragStart: handleDragStart, groups, onMoveToGroup: (sid: string, gid: string | null) => {
    ...
    return (
      <>
        ...
        {groups.map((g) => {
          ...
          return (
            <div key={g.id}>
              <GroupHeader group={g} isCollapsed={collapsedGroups.has(g.id)}
  ...
  return (
    <div className="flex h-full w-[200px] min-w-[200px] flex-col border-r border-border bg-bg-warm">
      ...
      <div className="flex items-center justify-between px-3 py-1.5">
        <span className="text-[12px] font-semibold text-text-secondary">{mode === 'chat' ? '对话' : 'Agents'}</span>
        <div className="flex items-center gap-0.5">
          {mode === 'agent' && (<Button variant="ghost" size="icon" onClick={handleCreateGroup} aria-label="New group"><FolderPlus className="h-3.5 w-3.5" /></Button>)}
          <Button variant="ghost" size="icon" onClick={onNewChat} aria-label="New session"><Plus className="h-3.5 w-3.5" /></Button>
        </div>
      </div>
      <SessionSearch value={searchQuery} onChange={setSearchQuery} />
      <ScrollArea className="flex-1 min-h-0">
        <div className="px-1.5 pb-3 space-y-[1px]">
          {filtered.length === 0 && (<p className="px-2 py-6 text-center text-[11px] text-text-tertiary">{searchQuery ? '没有匹配的对话' : `No ${mode} sessions yet`}</p>)}
          {mode === 'agent' && filtered.length > 0 ? renderAgentTree() : renderChatList()}
        </div>
      </ScrollArea>
      <div className="border-t border-border p-2">
        <button onClick={onOpenSettings} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-[12px] text-text-secondary transition-colors cursor-pointer hover:bg-bg-hover">
          <Settings className="h-3.5 w-3.5 shrink-0" />设置
        </button>
      </div>
      <ConfirmDialog open={deleteTarget !== null} title="删除会话" message="确定要删除吗？此操作无法撤销。"
        confirmLabel="删除" variant="danger" onConfirm={() => { onDeleteSession(deleteTarget!); setDeleteTarget(null); }} onCancel={() => setDeleteTarget(null)} />
    </div>
  );
}
