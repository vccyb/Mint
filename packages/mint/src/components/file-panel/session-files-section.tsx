  ...
  const handleUpload = useCallback(
    async (fileList: FileList) => {
      ...
      try {
        ...
        const res = await fetch('/api/sessions/files', { method: 'POST', body: formData });
        if (res.ok) {
          const json = await res.json();
    ...
    async (fileId: string, e: React.MouseEvent) => {
      ...
      try {
        const res = await fetch(
          `/api/sessions/files?sessionId=${encodeURIComponent(sessionId)}&fileId=${encodeURIComponent(fileId)}`,
          { method: 'DELETE' },
        );
        if (res.ok) {
