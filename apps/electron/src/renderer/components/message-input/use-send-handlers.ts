...
async function fetchMentionedFileContent(filePath: string): Promise<string | null> {
  // TODO: Needs an IPC handler for reading file content by path
  // For now, return null to skip file mention resolution
  try {
    // Placeholder: file content reading via IPC not yet implemented
    console.warn('fetchMentionedFileContent: IPC handler not implemented for file path:', filePath);
  } catch {
    // ignore
  }
  return null;
