...
async function ipcSearch(type: MentionType, q: string, projectId?: string | null): Promise<{ results: unknown[] }> {
  const api = (window as any).electronAPI;
  if (type === 'file') {
    return api.filesSearch({ q, projectId: projectId ?? undefined });
  } else if (type === 'skill') {
    return api.searchSkills({ q });
  } else {
    return { results: [] };
  }
}
  ...
  return {
    ...
    items: async ({ query }) => {
      try {
        ...
        const data = await ipcSearch(mentionType, q, currentProjectId);
        if (mentionType === 'file') {
          const raw = (data.results ?? []) as Array<{ name: string; path: string; type: string }>;
          return raw.map((r) => ({
            type: 'file' as const,
            label: r.name,
            value: r.path,
        ...
        return (data.results ?? []) as SuggestionItem[];
      } catch {
        return [];
      }
    },
