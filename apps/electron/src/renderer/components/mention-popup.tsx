...
async function ipcSearch(type: MentionType, q: string): Promise<{ results: unknown[] }> {
  const api = (window as any).electronAPI;
  if (type === 'file') {
    return api.filesSearch({ q });
  } else if (type === 'skill') {
    return api.searchSkills({ q });
  } else {
    // MCP tool search — stub for now
    return { results: [] };
  }
}
...
export function MentionPopup({ type, query, anchorRect, onSelect, onClose }: MentionPopupProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const q = query || '*';
        const data = await ipcSearch(type, q);
        if (type === 'file') {
          ...
          setResults((data.results ?? []) as MentionChip[]);
        }
      } catch {
